...
class Persona{
  ...
  //Vida máxima en años
  float diametroInicial=3;
  float diametroFinal=50;

  Body body; //Box2D body
  Vec2 pos; //Posición
  float diametro;
  float edad;
  float edadMaxima = random(100,800);
  ...
  Persona(){
   ...
   void draw(){
    ...
    f.getShape().m_radius= box2d.scalarPixelsToWorld(diametro*0.5*0.7);
    ...
    noFill();
    //Borde
    stroke(tonoPersona, saturacionPersona, brilloPersona, 120);
    strokeWeight(diametro);
    point(pos.x, pos.y);
    
    //Nucleo
    if (edad>edadMaxima*0.3){
      stroke(tonoPersona, saturacionPersona, brilloPersona, 180);
      strokeWeight(diametro*0.4);
      point(pos.x, pos.y);
    }
  ...
  void makeBody(float x, float y, float r) {
    ...
    cs.m_radius = box2d.scalarPixelsToWorld(r*0.7);
